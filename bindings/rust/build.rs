fn main() {
    ...
    c_config.compile("tree-sitter-query");

    println!("cargo:rustc-check-cfg=cfg(with_highlights_query)");
    if !"queries/query/highlights.scm".is_empty() && std::path::Path::new("queries/query/highlights.scm").exists() {
        println!("cargo:rustc-cfg=with_highlights_query");
    }
    println!("cargo:rustc-check-cfg=cfg(with_injections_query)");
    if !"queries/query/injections.scm".is_empty() && std::path::Path::new("queries/query/injections.scm").exists() {
